...
namespace WMS
{
    public partial class Fm_O005 : Form
    {
        // 多國語言資源檔設定
        System.Resources.ResourceManager RM = new System.Resources.ResourceManager(string.Format("WMS.Language.{0}.Resource_Fm_O005", "FORM_OUT"), System.Reflection.Assembly.GetExecutingAssembly());
        ...
        private void Fm_O005_Load(object sender, EventArgs e)
        {
        ...
        private void btnExec_Click(object sender, EventArgs e)
        {
            ...
            var result = TrkLib.CreateStkOut(Stks, cmbDevNo.Text);
        ...
        private void gdv_WMS_STK_CustomSummaryCalculate(object sender, DevExpress.Data.CustomSummaryEventArgs e)
        {
            if (gdc_WMS_STK.DataSource == null) return;
            if (gdv_WMS_STK.RowCount <= 0) return;

            if ((e.Item as DevExpress.XtraGrid.GridColumnSummaryItem).FieldName == "RES_QTY")
            {
                decimal TotalResQty = 0;
                List<Model.WmsStk> CheckList = ((List<Model.WmsStk>)gdc_WMS_STK.DataSource).Where(o => o.CHK == true).ToList();
                TotalResQty = CheckList.Sum(o => o.RES_QTY);
                e.TotalValue = string.Format("{1} {0}", TotalResQty.ToString("0.00"),RM.GetString("SubTotal"));
            }
        }
    }
}
