...
namespace WMS
{
    public partial class Fm_O002_1 : Form
    {
        // 多國語言資源檔設定
        ...
        public List<Model.WmsOutLine> SelectedList = new List<WMS.Model.WmsOutLine>();
        ...
        private void Fm_O002_1_Load(object sender, EventArgs e)
        {
            RefreshData("","");
            InitialListNoCombobox();
            SelectedList = new List<WMS.Model.WmsOutLine>();
        }
        ...
        private void InitialListNoCombobox()
        {
            ...
            cmbListNo.Properties.DataSource = OutLines
                .GroupBy(o => new
                {
                    LIST_NO = o.LIST_NO,
        ...
        {
            var result = ListService.GetAvailableWmsOutLine(ref OutLines);
            ...
            if (!string.IsNullOrEmpty(ListNo))
                OutLines = OutLines.Where(o => o.LIST_NO == ListNo).ToList();

            if (!string.IsNullOrEmpty(FilterString))
            {
                OutLines = OutLines.Where(o => (o.PROD_NO ?? string.Empty).Contains(FilterString) ||
            ...
            var test2NotInTest1 = OutLines.Where(t2 => SelectedList.Any(t1 => t2.LIST_NO == t1.LIST_NO && t2.LINE_ID == t1.LINE_ID));
        ...
        private void gdv_WMS_OUT_LINE_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            if (e.Column.FieldName == "CHK")
            {
                if (e.RowHandle >= 0)
                {
                    ...
                    string ListNo = gdv_WMS_OUT_LINE.GetRowCellValue(e.RowHandle, "LIST_NO").ToString();
                    int LineID = Convert.ToInt16(gdv_WMS_OUT_LINE.GetRowCellValue(e.RowHandle, "LINE_ID").ToString());
                    //勾選
                    if (value)
                    {
